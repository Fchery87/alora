...
export async function syncHandler(ctx: any, args: any) {
  const userOrgId = await requireOrganizationId(ctx);
...
export async function updateSettingsHandler(ctx: any, args: any) {
  const userOrgId = await requireOrganizationId(ctx);

  // Verify user's org matches requested org
  if (userOrgId !== args.clerkOrganizationId) {
    throw new Error("Not authorized to update settings for this organization");
  }
